...
source("Models/Adv_under_events.R")
...
monitors_pars <- c("lambda0","alpha","epsilon","sigma_epsilon","pi")
Sys.time()
mcmc.output <- nimbleMCMC(Adv_under_events, data = data, inits = inits, constants = constants,
...
# Check results
head(mcmc.output$summary)
tail(mcmc.output$summary)

# Pi estimation

df_plot=c()
pi=mcmc.output$samples[,grep("pi",colnames(mcmc.output$samples))]
for (W in seq(1,53,1)){
  aux=pi[,W]
  df_plot=rbind(df_plot,data.frame(W,
                                   Lo=quantile(aux,0.025),
                                   Mean=mean(aux),
                                   Up=quantile(aux,0.975)))
}
ggplot(df_plot,aes(x=W,y=Mean))+
  geom_ribbon(aes(ymin=Lo,ymax=Up),fill="gray60")+
  geom_line(linewidth=2)+
  theme_bw()+
  ylab("Week-level underestimation")+
